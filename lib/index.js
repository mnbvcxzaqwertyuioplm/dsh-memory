 ...
 *
 * 目标：让 DSH 自己像 jarvis 大脑那样"记得你"——通过 memory_add / memory_search
 * 两个工具做跨会话的语义召回。这跟 jarvis-bridge 里的记忆是**同一套逻辑**，但
 * 这个独立插件只关心"注册工具"，不关心 http 桥 / agents / session，因此能在
 * 任何带 tools 服务的 profile（web、headless）里随 dsh-base 之后安全加载。
 *
...
function apply(ctx, config = {}) {
	...
	try {
		...
		const cfg = {
			memoryDbPath: config.memoryDbPath ?? join(process.env.HOME || "/home/ubuntu", "workspace/Jarvis/data/jarvis-memory.db"),
			zhipuEnvPath: config.zhipuEnvPath ?? join(process.env.HOME || "/home/ubuntu", "workspace/Jarvis/.memenv"),
		...
		disposers.push(ok(() => {
			...
			return commands.register({
				name: "mem",
				description:
					"先检索跨会话长期记忆（含 Jarvis 项目全部知识）再把结果交给 agent 回答；当你不确定 agent 会不会主动回忆时用它。",
				input: { hint: "要回忆的问题，如：jarvis 的兄弟叫什么" },
				handler: async (invocation) => {
					try {
						const query = String(invocation.rawInput || "").trim();
						if (!query) return { kind: "error", text: "用法：/mem <问题>，例如 /mem jarvis 的兄弟叫什么" };
