			...
			return commands.register({
				...
				handler: async (invocation) => {
					try {
						...
						// 必须把用户原始问题带进消息：命令机制只把 /mem 后的文本交给 handler，
						// 不会传给 agent——否则 agent 看到结果却不知道用户问了什么。
						const text = `用户刚才的问题是：${query}\n\n跨会话记忆检索结果（${results.length} 条）：\n${lines.join("\n")}\n\n请基于以上记忆直接回答用户的问题；若命中隐私/约定类条目，须一并遵守。`;
						// 若 agent 支持 followup，主动驱动一轮基于记忆的回答（用户能直接看到回复）；
						// 否则退化为只返回检索结果文本。
						if (typeof invocation.agent?.followup === "function" && typeof invocation.agent?.whenIdle === "function") {
							try {
								// 消息必须带 source.kind（agent 处理时会读 message.source.kind），
								// 否则抛 "Cannot read properties of undefined (reading 'kind')"。
								// 优先用官方 createUserMessage（dsh-llm 为 dsh-base 依赖，回退层必在）；
								// 解析失败则手写带 source 的等价格式。
								let msg;
								try {
									...
									msg = { role: "user", id: `msg-${randomUUID()}`, content: [{ type: "text", text }], source: { kind: "user" } };
								}
								invocation.agent.followup(msg);
								await invocation.agent.whenIdle();
								return { kind: "success", text: "已检索记忆并基于结果回答（见上方回复）。" };
							} catch (err) {
								return { kind: "error", text: `记忆检索成功，但驱动回答失败：${err instanceof Error ? err.message : String(err)}` };
							}
						}
		...
		if (config.recallHint !== false) {
			disposers.push(ok(() => {
				...
				return sp.section({
					...
					order: 61,
					text:
						"你拥有跨会话长期记忆工具 memory_search：回答任何可能涉及过往会话内容的问题" +
						"（项目历史、架构决策、个人背景、过往约定、踩坑教训、之前聊过的事）前，" +
